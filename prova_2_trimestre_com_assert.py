...
def cadastrar_produtora():
    try:
        ...
        cursor = conexao.cursor()
        cursor.execute(
            "INSERT INTO produtoras (razao_social, telefone_comercial) VALUES (?, ?)",
            (razaosocial, telefonecomercial))
...
def atualizar_produtora():
    try:
        ...
        cursor.execute(
            "UPDATE produtoras SET razao_social = ?, telefone_comercial = ? WHERE id = ?",
            (nova_razao_social, novo_telefone_comercial, id_produtora))
...
def excluir_produtora():
    try:
        ...
        cursor.execute(
            "SELECT id FROM produtoras WHERE id = ?",
            (id_produtora,))
        ...
        if not produtora:
            ...
            cursor.execute(
                "DELETE FROM produtoras WHERE id = ?",
                (id_produtora,))
...
def cadastrar_arena():
    try:
        ...
        if not produtora:
            ...
            cursor.execute(
                "INSERT INTO arenas_estadios "
                "(nome_local, capacidade_maxima, id_produtora) "
                "VALUES (?, ?, ?)",
                (nome_local, capacidade_maxima, id_produtora))
...
def atualizar_arena():
    try:
        ...
        if not produtora:
            ...
            cursor.execute(
                "UPDATE arenas_estadios SET nome_local = ?, "
                "capacidade_maxima = ?, id_produtora = ? WHERE id = ?",
                (novo_nome_local, nova_capacidade_maxima,
                 id_produtora, id_arena))
...
def excluir_arena():
    try:
        ...
        cursor.execute(
            "SELECT id FROM arenas_estadios WHERE id = ?",
            (id_arena,))
        ...
        if not arena:
            ...
            cursor.execute(
                "DELETE FROM arenas_estadios WHERE id = ?",
                (id_arena,))
...
assert inicializar_banco() is not None



assert cadastrar_produtora is not None
assert callable(cadastrar_produtora)



assert listar_produtoras is not None
assert callable(listar_produtoras)



assert atualizar_produtora is not None
assert callable(atualizar_produtora)



assert excluir_produtora is not None
assert callable(excluir_produtora)



assert cadastrar_arena is not None
assert callable(cadastrar_arena)



assert listar_arenas is not None
assert callable(listar_arenas)



assert atualizar_arena is not None
assert callable(atualizar_arena)



assert excluir_arena is not None
assert callable(excluir_arena)
